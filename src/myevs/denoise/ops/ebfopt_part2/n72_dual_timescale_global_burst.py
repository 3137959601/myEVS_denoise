...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n72 requires numba, but import failed")


def _try_build_n72_kernel():
    _require_numba()
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            score = e_dual / (1.0 + lambda_eff * float(inhib_self[idx]))
...
def score_stream_n72(
    ...
    *,
    ...
    """N7.2: N7.1 with global burst modulation.

    G(t) = clip(R_short / (R_long + eps) - 1, 0, 1)
    lambda_eff = lambda0 * (1 + alpha * G)
    rho_eff = rho0 * (1 - beta * G)

    score_i = (E_fast + rho_eff * max(0, E_slow - E_fast)) / (1 + lambda_eff * I_self)

    Env vars:
    - MYEVS_N72_TAU0_US (default 16000)
    - MYEVS_N72_TAU_INHIB_US (default 32000)
    - MYEVS_N72_SLOW_RATIO (default 4.0)
    - MYEVS_N72_FOOT_DT_MAX_RATIO (default 8.0)
    - MYEVS_N72_RHO0 (default 0.60)
    - MYEVS_N72_SELF_GAIN (default 0.55)
    - MYEVS_N72_LAMBDA0 (default 0.30)
    - MYEVS_N72_SIGMA_SPATIAL (default 2.20)
    - MYEVS_N72_G_TAU_SHORT_US (default 2000)
    - MYEVS_N72_G_TAU_LONG_US (default 20000)
    - MYEVS_N72_G_ALPHA (default 0.80)
    - MYEVS_N72_G_BETA (default 0.30)
    """
    ...
    tau0_us = int(_read_int_env("MYEVS_N72_TAU0_US", 16000))
    tau_inhib_us = int(_read_int_env("MYEVS_N72_TAU_INHIB_US", 32000))
    slow_ratio = float(_read_float_env("MYEVS_N72_SLOW_RATIO", 4.0))
    dt_max_ratio = float(_read_float_env("MYEVS_N72_FOOT_DT_MAX_RATIO", 8.0))
    ...
    rho0 = float(_read_float_env("MYEVS_N72_RHO0", 0.60))
    self_gain = float(_read_float_env("MYEVS_N72_SELF_GAIN", 0.55))
    lambda0 = float(_read_float_env("MYEVS_N72_LAMBDA0", 0.30))
    sigma_spatial = float(_read_float_env("MYEVS_N72_SIGMA_SPATIAL", 2.20))

    g_tau_short_us = int(_read_int_env("MYEVS_N72_G_TAU_SHORT_US", 2000))
    g_tau_long_us = int(_read_int_env("MYEVS_N72_G_TAU_LONG_US", 20000))
    g_tau_short_ticks = float(tb.us_to_ticks(max(1, g_tau_short_us)))
    g_tau_long_ticks = float(tb.us_to_ticks(max(1, g_tau_long_us)))
    g_alpha = float(_read_float_env("MYEVS_N72_G_ALPHA", 0.80))
    g_beta = float(_read_float_env("MYEVS_N72_G_BETA", 0.30))

    ker = _try_build_n72_kernel()
    scores = ker(
