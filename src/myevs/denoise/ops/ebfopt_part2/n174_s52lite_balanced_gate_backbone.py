...
# Balanced-gate constants (fixed, no user knobs).
N174_SIGMA = 2.8
N174_RSTATE_INIT = 0.10
N174_B_INIT = 0.50


def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n174 requires numba, but import failed")
...
def _try_build_n174_kernel():
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            opp_rel = purity * (1.0 - mstate) * (1.0 - rstate)
            if opp_rel < 0.0:
                opp_rel = 0.0
            if opp_rel > 1.0:
                opp_rel = 1.0
            opp_rel = opp_rel * opp_rel
            ...
            support_gain = 1.0 + b * sfrac * (1.0 - rstate)
            if support_gain < 1.0:
                support_gain = 1.0
            if support_gain > 2.0:
                support_gain = 2.0
...
def score_stream_n174(
    ...
    *,
    ...
    """N174: balanced minimal gate (coefficient-light, state-aware).

    Key points:
    - Uses three slow scalars (mix/rhythm/support envelope), no extra per-pixel arrays.
    - Uses coefficient-light multiplicative terms (mainly 0/1 and square).
