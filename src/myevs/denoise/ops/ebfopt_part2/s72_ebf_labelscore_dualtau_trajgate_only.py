...
def try_build_s72_ebf_labelscore_dualtau_trajgate_only_scores_kernel():
    r"""Build and return Numba kernel for s72 score streaming.

    s72 goal:
    - Extremely simplified variant that keeps only ONE novel ingredient:
      a trajectory gate computed from same-polarity centroid drift between
      short/long time windows.

    What is removed vs s63-s71:
    - No self-occupancy / hot_state.
    - No opposite-polarity mixing terms or mix_state.
    - No beta/auto-beta.
    - No purity/mix gates.
    - No delta cap and no final divide-by-(1+u^2).
    ...
    - Trajectory gate from centroid drift:
        c_traj = clip( ||c_long - c_short|| / radius_px, 0, 1 )
    - Delta-only long boost gated by c_traj:
        score = raw_short + c_traj * max(0, raw_long - raw_short)
    ...
    def ebf_s72_scores_stream(
        ...
        for i in range(n):
            ...
            scores_out[i] = raw_short + c_traj * delta

    return ebf_s72_scores_stream
