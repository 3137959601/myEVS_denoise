...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n150 requires numba, but import failed")
...
def _try_build_n150_kernel():
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            base_score = raw_gated / (1.0 + (u_lite * u_lite))
...
def score_stream_n150(
    ...
    *,
    ...
    """N150: n149 compact-LUT backbone + S52-lite fusion without dense hot_state.

    Key changes relative to n149:
    - Removes per-pixel dense hot_state table.
    - Uses dt0-based u_lite as a lightweight self-occupancy proxy.
    - Keeps S52-style global mix gate and support boost.
    """
    ...
    sigma_space = float(_read_float_env("MYEVS_N150_SIGMA", 2.5))
    beta_init = float(_read_float_env("MYEVS_N150_BETA_INIT", 0.5))
    if sigma_space <= 1e-6:
        sigma_space = 1e-6
