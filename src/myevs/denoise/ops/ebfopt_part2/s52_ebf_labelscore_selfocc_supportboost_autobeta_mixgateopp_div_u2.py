...
def try_build_s52_ebf_labelscore_selfocc_supportboost_autobeta_mixgateopp_div_u2_scores_kernel():
    """Build and return Numba kernel for s52 score streaming.

    s52 goal: approach "light/mid ~= s21" and "heavy ~= s51" without adding
    environment-sensitive hyperparameters and without running two kernels.

    Observation from diagnostics (ED24/myPedestrain_06, prescreen200k, s=7,tau=64ms):
    - light shows much LOWER neighborhood polarity mix (opp/sum) than heavy.
    - heavy shows much HIGHER polarity mix.

    Idea:
    - Keep s51 core:
        base = raw_same / (1 + u_self^2)
        score = base * (1 + beta_state * sfrac)
      where beta_state is an online running mean of u_self and
      sfrac is the neighbor support fraction.

    - Add opposite-polarity neighborhood evidence (opp_w) like s21, but gate it
      automatically using an online running mean of polarity mix (mix_state):

        mix = opp_w / (raw_w + opp_w + eps)
        mix_state <- mix_state + (mix - mix_state) / N
        alpha_eff = (1 - mix_state)^2

      Then use gated raw:

        raw_gated = (raw_w + alpha_eff * opp_w) / tau

      This makes opp evidence stronger in low-mix scenes (typical light/mid)
      and weaker in high-mix scenes (typical heavy), without exposing knobs.
    ...
    def ebf_s52_scores_stream(
        ...
        for i in range(n):
            ...
            # Per-event polarity mix and its running mean.
