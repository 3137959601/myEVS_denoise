...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n144 requires numba, but import failed")
        ...
        for i in range(n):
            ...
            for yy in range(y0, y1 + 1):
                ...
                for xx in range(x0, x1 + 1):
                    ...
                    # 7.87 ablation: remove the square in temporal kernel.
...
def score_stream_n144(
    ...
    *,
    ...
    """N144: bilateral Gaussian with linear temporal kernel.

    - temporal: max(0, 1 - dt/tau)
    ...
    sigma_space = float(_read_float_env("MYEVS_N144_SIGMA", 3.0))
    lut_space = _build_space_lut(int(radius_px), float(sigma_space))

    ker = _try_build_n144_kernel()
    ker(
