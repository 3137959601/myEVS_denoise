...
def try_build_s70_ebf_labelscore_dualtau_condlong_selfocc_trajgate_mixgateopp_deltacap2_puritygate_div_u2_scores_kernel():
    """Build and return Numba kernel for s70 score streaming.

    s70 builds on s69:
    - Keep s63 trajectory gate and delta cap=2*raw_short.
    - Add a polarity-purity gate based on the short-window mix ratio:
        mix = opp_w / (raw_w + opp_w)
        c_pol = clip(1 - 2*mix, 0, 1)
      and apply it to long boost:
        g_long = (1 - u_self) * c_traj * c_pol

    Hypothesis:
    - The extra long-delta allowed by cap2 helps recall, but also leaks FP when
      local polarity mix is high (noise-like). Purity gating should recover
      precision without giving up much signal.
    ...
    def ebf_s70_scores_stream(
        ...
        for i in range(n):
            ...
            # Purity gate: 1 at mix=0, 0 at mix>=0.5.
            c_pol = 1.0 - 2.0 * mix
            if c_pol < 0.0:
                c_pol = 0.0
            if c_pol > 1.0:
            ...
            g_long = (1.0 - u_self) * c_traj * c_pol
            if g_long < 0.0:
                g_long = 0.0
            if g_long > 1.0:
                g_long = 1.0
