...
# Minimal-form constants (fixed, no user knobs).
N172_SIGMA = 2.8
N172_RSTATE_INIT = 0.10


def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n172 requires numba, but import failed")
...
def score_stream_n172(
    ...
    *,
    ...
    """N172: minimal-form rhythm gate with interpretable multiplicative factors.

    Key points:
    - Uses only two slow scalars: opposite-mix state and flip-rhythm state.
    - Removes hand-tuned affine coefficients and keeps algebraic 0/1 structure.
