...
# Fixed constants (no new user hyperparameters).
N170_SIGMA = 2.8
N170_TSTATE_INIT = 0.25


def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n170 requires numba, but import failed")
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            if prev_pol0 == -pi:
                flip = 1.0
            q = flip * u_lite
            tstate = tstate + (q - tstate) / N
            if tstate < 0.0:
                tstate = 0.0
            if tstate > 1.0:
                tstate = 1.0
            transition_pressure = 0.5 * (tstate + q)
            if transition_pressure < 0.0:
                transition_pressure = 0.0
            if transition_pressure > 1.0:
            ...
            lambda_opp = (1.0 - mix) * (1.0 - transition_pressure)
            if lambda_opp < 0.0:
                lambda_opp = 0.0
            if lambda_opp > 1.0:
                lambda_opp = 1.0
            lambda_opp = lambda_opp * lambda_opp

            u_eff = u_lite * (1.0 + 0.9 * transition_pressure) * (1.0 - 0.45 * sfrac)
            if u_eff < 0.0:
                u_eff = 0.0
            if u_eff > 1.0:
                u_eff = 1.0

            support_gain = 1.0 + 0.55 * sfrac * (1.0 - transition_pressure)
            if support_gain < 1.0:
                support_gain = 1.0
            if support_gain > 1.55:
                support_gain = 1.55

            score_core = raw_same + lambda_opp * raw_opp
...
def score_stream_n170(
    ...
    *,
    ...
    """N170: polarity-transition memory reformulation (non-S52, fixed constants).

    Key points:
    - Uses n149 compact-LUT evidence extraction (same O(r^2) neighborhood loop).
    - Replaces S52 mix/beta fusion with transition-pressure memory from center
      polarity flips.
    - No additional per-pixel persistent arrays.
    """
    ...
    sigma_space = float(N170_SIGMA)
    tstate_init = float(N170_TSTATE_INIT)
    if sigma_space <= 1e-6:
        sigma_space = 1e-6
