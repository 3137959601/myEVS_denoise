...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n101 requires numba, but import failed")


def _try_build_n101_kernel():
    _require_numba()
...
def score_stream_n101(
    ...
    *,
    ...
    """N101: top3 2D mean compatibility times support factor."""
    ...
    d0_px = float(_read_float_env("MYEVS_N101_D0_PX", 2.2))
    t0_us = float(_read_float_env("MYEVS_N101_T0_US", 1400.0))
    sigma_d_px = float(_read_float_env("MYEVS_N101_SIGMA_D_PX", 1.2))
    sigma_t_us = float(_read_float_env("MYEVS_N101_SIGMA_T_US", 5000.0))
    support_n0 = float(_read_float_env("MYEVS_N101_SUPPORT_N0", 5.0))

    ker = _try_build_n101_kernel()
    ker(
