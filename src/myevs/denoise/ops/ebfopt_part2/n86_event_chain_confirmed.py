...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n86 requires numba, but import failed")


def _try_build_n86_kernel():
    _require_numba()
        ...
        for i in range(n):
            ...
            if best_k >= 0 and best_d < d_new:
                ...
                if st == 1:
                    ...
                    if int(new_len) >= int(confirm_len_min) and disp >= confirm_disp_min and cond_hot:
...
def score_stream_n86(
    ...
    *,
    ...
    """N86 (7.40 ECSM-C): confirmed-chain decision model.
    ...
    tau_fast_us = int(_read_int_env("MYEVS_N86_TF_US", tau_fast_default))
    tau_slow_us = int(_read_int_env("MYEVS_N86_TS_US", tau_slow_default))
    tau_inhib_us = int(_read_int_env("MYEVS_N86_TAU_INHIB_US", tau_inhib_default))

    tau_confirm_us = int(_read_int_env("MYEVS_N86_T_CONFIRM_US", tau_slow_us))
    tau_kill_us = int(_read_int_env("MYEVS_N86_T_KILL_US", max(2000, 2 * tau_slow_us)))
    ...
    self_gain = float(_read_float_env("MYEVS_N86_SELF_GAIN", 0.55))

    foot_sigma = float(_read_float_env("MYEVS_N86_FOOT_SIGMA", 2.2))
    foot_rho = float(_read_float_env("MYEVS_N86_FOOT_RHO", 0.60))
    foot_dt_max_ratio = float(_read_float_env("MYEVS_N86_FOOT_DT_MAX_RATIO", 2.0))
    foot_dt_max_ticks = float(max(1.0, tau_slow_ticks * max(1.0, foot_dt_max_ratio)))

    c_hot = float(_read_float_env("MYEVS_N86_C_HOT", 1.0))
    c_foot = float(_read_float_env("MYEVS_N86_C_FOOT", 0.8))

    r0_px = float(_read_float_env("MYEVS_N86_R0_PX", 2.0))
    kv_px_per_ms = float(_read_float_env("MYEVS_N86_KV_PX_PER_MS", 0.10))
    kv_px_per_tick = kv_px_per_ms / max(1e-6, float(tb.us_to_ticks(1000.0)))

    d_new = float(_read_float_env("MYEVS_N86_D_NEW", 1.2))
    alpha_slow = float(_read_float_env("MYEVS_N86_ALPHA_SLOW", 0.80))

    w_x = float(_read_float_env("MYEVS_N86_W_X", 1.0))
    w_t = float(_read_float_env("MYEVS_N86_W_T", 0.6))
    w_h = float(_read_float_env("MYEVS_N86_W_H", 0.5))
    w_f = float(_read_float_env("MYEVS_N86_W_F", 0.7))

    track_capacity = int(_read_int_env("MYEVS_N86_TRACK_CAPACITY", 1024))
    track_buf_max = int(_read_int_env("MYEVS_N86_TRACK_BUF_MAX", 32))

    confirm_len_min = int(_read_int_env("MYEVS_N86_CONFIRM_LEN_MIN", 3))
    confirm_disp_min = float(_read_float_env("MYEVS_N86_CONFIRM_DISP_MIN", 2.0))
    confirm_hot_max = float(_read_float_env("MYEVS_N86_CONFIRM_HOT_MAX", 1.0))

    k_len = float(_read_float_env("MYEVS_N86_K_LEN", 3.0))
    q_l_w = float(_read_float_env("MYEVS_N86_Q_L_W", 0.25))
    q_p_w = float(_read_float_env("MYEVS_N86_Q_P_W", 0.40))
    q_h_w = float(_read_float_env("MYEVS_N86_Q_H_W", 0.20))
    q_f_w = float(_read_float_env("MYEVS_N86_Q_F_W", 0.15))
    q_min = float(_read_float_env("MYEVS_N86_Q_MIN", 0.0))
    q_max = float(_read_float_env("MYEVS_N86_Q_MAX", 1.0))

    ker = _try_build_n86_kernel()
    scores = ker(
