...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n152 requires numba, but import failed")
...
def _try_build_n152_kernel():
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            # Same-state proxy:
            # - keep n150's dt0-based core u_lite
            # - relax suppression in strong-support regions (high sfrac)
            # - reinforce suppression in mixed-polarity local neighborhoods (high mix)
            relief = 1.0 - ks * sfrac
            if relief < 0.25:
                relief = 0.25
            if relief > 1.0:
...
def score_stream_n152(
    ...
    *,
    ...
    """N152: n150 backbone with support/mix-adaptive center proxy.

    Key changes relative to n150:
    - Keeps zero extra per-pixel state (same memory footprint as n150).
    - Reuses dt0-based u_lite, but adapts it with local support fraction (sfrac)
      and local polarity-mix (mix) before center suppression.
    - Keeps the same S52-lite global mix gate and support boost structure.
    """
    ...
    sigma_space = float(_read_float_env("MYEVS_N152_SIGMA", 2.8))
    beta_init = float(_read_float_env("MYEVS_N152_BETA_INIT", 0.65))
    k_sfrac = float(_read_float_env("MYEVS_N152_K_SFRAC", 0.45))
    k_mix = float(_read_float_env("MYEVS_N152_K_MIX", 0.35))
    if sigma_space <= 1e-6:
        sigma_space = 1e-6
