...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n121 requires numba, but import failed")


def _try_build_n121_kernel():
    _require_numba()
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            score_final = e_core - b * e_surround
            if score_final < 0.0:
                score_final = 0.0

            out[i] = np.float32(score_final)
...
def score_stream_n121(
    ...
    *,
    ...
    """N121 (7.67): spatiotemporal center-surround antagonism."""
    ...
    r_core = int(_read_int_env("MYEVS_N121_R_CORE", 1))
    beta = float(_read_float_env("MYEVS_N121_BETA", 0.2))

    ker = _try_build_n121_kernel()
    ker(
