...
N182_SIGMA = 2.8
N182_BETA_INIT = 0.65
N182_K_SFRAC = 2.0 / 3.0
N182_K_MIX = 1.0 / 5.0
N182_RSTATE_INIT = 0.10
N182_RHYTHM_GOOD_COEFF = 0.75
N182_PI_LAMBDA = 0.25
N182_MODE_CONSERVATIVE = 1
N182_MODE_MINIMAL = 0

_N182_KERNEL = None
_N182_TABLE_CACHE: dict[tuple[int, float], tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]] = {}


def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("N182 requires numba, but import failed")
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            if relief > 1.0:
                relief = 1.0
            rhythm_scale = 1.0
            if mode == N182_MODE_CONSERVATIVE:
                rhythm_scale = 1.0 - rgood_coeff * rhythm_good
            if rhythm_scale < 0.5:
                rhythm_scale = 0.5
            if rhythm_scale > 1.75:
                rhythm_scale = 1.75
            pi_scale = 1.0 + lpi * rhythm_pressure
            if pi_scale < 1.0:
                pi_scale = 1.0
            if pi_scale > 1.5:
                pi_scale = 1.5
            u_eff = u_lite * relief * rhythm_scale * pi_scale
            if u_eff < 0.0:
                u_eff = 0.0
            if u_eff > 1.0:
                u_eff = 1.0
...
def _get_n182_kernel():
    global _N182_KERNEL
    if _N182_KERNEL is None:
        _N182_KERNEL = _try_build_n182_kernel()
    return _N182_KERNEL
...
def _get_compact_tables_cached(
    ...
    if sig <= 1e-6:
        sig = 1e-6
    key = (rr, sig)
    cached = _N182_TABLE_CACHE.get(key)
    ...
    _N182_TABLE_CACHE[key] = cached
    return cached


def _score_stream_n182_core(
    ev,
    *,
    width: int,
    height: int,
    ...
    k_sfrac: float,
    k_mix: float = 0.0,
    rhythm_good_coeff: float = N182_RHYTHM_GOOD_COEFF,
    pi_lambda: float = N182_PI_LAMBDA,
    simplify_mode: int = N182_MODE_CONSERVATIVE,
...
def score_stream_n182(
    ...
    *,
    ...
    """N182: simplified n179 candidate.

    conservative: u' = u * (1 - k_s*s) * (1 - r_g*good) * (1 + lambda*pi)
    minimal:      u' = u * (1 - k_s*s) * (1 + lambda*pi)
    """

    m = str(mode).strip().lower()
    simplify_mode = N182_MODE_MINIMAL if m in {"minimal", "min"} else N182_MODE_CONSERVATIVE

    return _score_stream_n182_core(
        ev,
        width=int(width),
        height=int(height),
        ...
        tau_us=int(tau_us),
        tb=tb,
        beta_init=float(N182_BETA_INIT if beta_init is None else beta_init),
        k_sfrac=float(N182_K_SFRAC if k_sfrac is None else k_sfrac),
        k_mix=0.0,
        rhythm_good_coeff=float(N182_RHYTHM_GOOD_COEFF if rhythm_good_coeff is None else rhythm_good_coeff),
        pi_lambda=float(N182_PI_LAMBDA if pi_lambda is None else pi_lambda),
