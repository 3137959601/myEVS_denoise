...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n117 requires numba, but import failed")


def _try_build_n117_kernel():
    _require_numba()
            ...
            for yy in range(y0, y1 + 1):
                ...
                for xx in range(x0, x1 + 1):
                    ...
                    if pj == pi:
                        ...
                        if dt_ticks >= e_min and dt_ticks <= e_max:
                            has_echo = True

            if has_echo and alpha > 0.0:
                score_base *= (1.0 + alpha)
...
def score_stream_n117(
    ...
    *,
    ...
    """N117 (7.63): bipolar spatiotemporal echo boost."""
    ...
    echo_min_us = int(_read_int_env("MYEVS_N117_ECHO_MIN_US", 1000))
    echo_max_us = int(_read_int_env("MYEVS_N117_ECHO_MAX_US", 100000))
    echo_min_ticks = int(tb.us_to_ticks(int(echo_min_us)))
    echo_max_ticks = int(tb.us_to_ticks(int(echo_max_us)))
    alpha_boost = float(_read_float_env("MYEVS_N117_ALPHA", 0.5))

    ker = _try_build_n117_kernel()
    ker(
