...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n100 requires numba, but import failed")


def _try_build_n100_kernel():
    _require_numba()
...
def score_stream_n100(
    ...
    *,
    ...
    """N100: top3 2D max compatibility times support factor."""
    ...
    d0_px = float(_read_float_env("MYEVS_N100_D0_PX", 2.2))
    t0_us = float(_read_float_env("MYEVS_N100_T0_US", 1400.0))
    sigma_d_px = float(_read_float_env("MYEVS_N100_SIGMA_D_PX", 1.2))
    sigma_t_us = float(_read_float_env("MYEVS_N100_SIGMA_T_US", 5000.0))
    support_n0 = float(_read_float_env("MYEVS_N100_SUPPORT_N0", 5.0))

    ker = _try_build_n100_kernel()
    ker(
