...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("s85 requires numba, but import failed")


def _try_build_s85_kernel():
    _require_numba()
    ...
    def _kernel(
        ...
        # Quantized replacement for s52 hot_state(int32).
        hot_u8 = np.zeros((npx,), dtype=np.uint8)
        ...
        for i in range(n):
            ...
            # Quantize dt into 0..64 bins, then do u8 leaky update.
            ...
            hot_u8[idx0] = np.uint8(h0)

            # Map quantized occupancy to [0,1] and keep s52 fusion topology.
...
def score_stream_s85(
    ...
    *,
    ...
    """S85: S52 with quantized uint8 hot-state.

    Keeps S52's fusion behavior but compresses per-pixel hot occupancy state:
    hot_state int32 -> hot_u8 uint8.
    """
