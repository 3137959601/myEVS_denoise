...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n143 requires numba, but import failed")
        ...
        for i in range(n):
            ...
            for yy in range(y0, y1 + 1):
                ...
                for xx in range(x0, x1 + 1):
                    ...
                    # 7.87 approximation for temporal Gaussian.
                    ratio = float(dt_ticks) * inv_tau_base
                    w_time = 1.0 - ratio * ratio
                    if w_time <= 0.0:
                        continue
...
def score_stream_n143(
    ...
    *,
    ...
    """N143 (7.87): bilateral Gaussian approximation.
    ...
    sigma_space = float(_read_float_env("MYEVS_N143_SIGMA", 3.0))
    lut_space = _build_space_lut(int(radius_px), float(sigma_space))

    ker = _try_build_n143_kernel()
    ker(
