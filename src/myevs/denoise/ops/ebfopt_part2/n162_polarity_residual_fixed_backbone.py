...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n162 requires numba, but import failed")
...
def _try_build_n162_kernel():
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            # Non-S52 polarity reformulation (residual contrast variant):
            # - same-polarity evidence is the base.
            # - add a bounded enhancement from positive same-vs-opp residual.
            # - keep support-based confidence gain and moderate center suppression.
            etot = raw_same + raw_opp
            if etot <= 0.0:
                out[i] = np.float32(0.0)
                continue

            residual = raw_same - raw_opp
            if residual < 0.0:
                residual = 0.0
            raw_core = raw_same + 0.75 * residual

            support_gain = 1.0 + 0.4 * sfrac
            u_eff = u_lite * (1.0 - 0.4 * sfrac)
            if u_eff < 0.0:
                u_eff = 0.0
            if u_eff > 1.0:
                u_eff = 1.0

            out[i] = np.float32((raw_core * support_gain) / (1.0 + u_eff * u_eff))

    return _kernel


def score_stream_n162(
    ev,
    *,
    width: int,
    height: int,
    ...
    """N162: compact-lut backbone + residual polarity reformulation (non-S52).
