...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n107 requires numba, but import failed")


def _try_build_n107_kernel():
    _require_numba()
...
def score_stream_n107(
    ...
    *,
    ...
    """N107 (7.54): Projected Axis Energy (Soft Projection) without N0 scale."""
    ...
    sigma_d_px = float(_read_float_env("MYEVS_N107_SIGMA_D_PX", 1.8))
    gamma_iso = float(_read_float_env("MYEVS_N107_GAMMA_ISO", 1.0))

    ker = _try_build_n107_kernel()
    ker(
