...
def try_build_s66_ebf_labelscore_dualtau_condlong_selfocc_distgate_mixgateopp_div_u2_scores_kernel():
    """Build and return Numba kernel for s66 score streaming.

    s66 is a lightweight follow-up after s65's failure:
    - Keep conditional long-delta (short=tau, long=2*tau, delta-only boost).
    - Use a more stable trajectory proxy than direction-consistency:
        c_dist = clip(dist_to_nearest_samepol / radius_px, 0, 1)
      where the nearest same-pol neighbor is searched under the short window.
    - Gate:
        g_long = (1 - u_self) * c_dist
