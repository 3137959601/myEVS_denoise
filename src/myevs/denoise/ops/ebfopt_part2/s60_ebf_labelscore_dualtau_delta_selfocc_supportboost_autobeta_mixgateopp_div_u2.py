...
def try_build_s60_ebf_labelscore_dualtau_delta_selfocc_supportboost_autobeta_mixgateopp_div_u2_scores_kernel():
    """Build and return Numba kernel for s60 score streaming.

    s60 goal:
    - Improve robustness in sparse / low-SNR segments (notably heavy seg1) while
      avoiding regressions on light/mid.
    - Streaming, single pass, O(r^2) neighborhood scan.
    - Minimal state and minimal hyperparameters: reuse s52 style (hot_state +
      beta_state + mix_state) and do not expose new knobs.

    Core idea:
    - Compute neighborhood evidence on two time scales using the same per-pixel
      last timestamp:
        short: tau
        long : 2*tau (fixed ratio, non-exposed)
    - Gate opposite-polarity evidence using adaptive polarity-mix running mean
      (same as s52): alpha_eff=(1-mix_state)^2.
    - Use a delta-only long-term boost to avoid changing dense regimes:
        raw = raw_short + clamp(max(0, raw_long - raw_short), 0, raw_short)
      This primarily helps when short-term support is weak but longer support
      still exists.
    - Suppress hot pixels via self-occupancy u_self from hot_state:
        score = raw / (1 + u_self^2)
    - Restore signal when neighborhood support is broad using s51/s52 style
      support-fraction boost with adaptive beta_state.
    ...
    def ebf_s60_scores_stream(
        ...
        for i in range(n):
            ...
            # Delta-only boost from long evidence.
            ...
            raw_dual = raw_short + delta

            base_score = float(raw_dual / (1.0 + (u_self * u_self)))
