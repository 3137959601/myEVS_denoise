...
def try_build_s54_ebf_labelscore_selfocc_supportboost_autobeta_eventmixgateopp_root4_div_u2_scores_kernel():
    """Build and return Numba kernel for s54 score streaming.

    s54 is a shape tweak of s53 (no new hyperparameters):

    - s53: alpha_eff = (1-mix)^2 * sqrt(sfrac)
    - s54: alpha_eff = (1-mix)^2 * sfrac^(1/4)

    Motivation:
    - sqrt(sfrac) can be overly conservative when sfrac is small (typical for larger
      windows), potentially under-utilizing opp evidence for true signal in light/mid.
    - Use a smoother dependence on support width (fourth root) to recover some signal
      while still suppressing narrow-support toggle/flicker noise.
    ...
    def ebf_s54_scores_stream(
        ...
        for i in range(n):
            ...
            if sfrac > 0.0:
                # sfrac^(1/4) = sqrt(sqrt(sfrac))
                alpha_eff = alpha_eff * math.sqrt(math.sqrt(sfrac))
            else:
                alpha_eff = 0.0
