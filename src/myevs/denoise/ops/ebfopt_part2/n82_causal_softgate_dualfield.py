...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n82 requires numba, but import failed")


def _try_build_n82_kernel():
    _require_numba()
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            # Soft trust in trajectory evidence:
            # conf -> 0 means rely on dual-footprint baseline; conf -> 1 means apply stronger traj gate.
            conf = traj / (traj + max(eps, traj_conf_k))
            if conf < traj_conf_floor:
                conf = traj_conf_floor
            if conf > 1.0:
                conf = 1.0
            ...
            score = (soft_gate * e_dual) / (1.0 + lambda_eff * float(inhib_self[idx]))
...
def score_stream_n82(
    ...
    *,
    ...
    """N8.2: data-driven soft trajectory gating on dual-field score.

    Motivation:
    - N8.1 showed over-suppression (precision up, recall down), especially mid/heavy.
    - Heavy seg1 is sparse; uncertain trajectory evidence should not hard-suppress score.

    score_i = soft_gate(C_traj) * E_dual / (1 + lambda_eff * I_self)

    Env vars:
    - MYEVS_N82_TAU0_US (default 16000)
    - MYEVS_N82_TAU_INHIB_US (default 32000)
    - MYEVS_N82_SLOW_RATIO (default 4.0)
    - MYEVS_N82_FOOT_DT_MAX_RATIO (default 8.0)
    - MYEVS_N82_RHO0 (default 0.60)
    - MYEVS_N82_SELF_GAIN (default 0.55)
    - MYEVS_N82_LAMBDA0 (default 0.30)
    - MYEVS_N82_SIGMA_SPATIAL (default 2.20)
    - MYEVS_N82_TAU_TRAJ_US (default 12000)
    - MYEVS_N82_TAU_TRAJ_CONS_US (default 8000)
    - MYEVS_N82_TRAJ_DT_MAX_US (default 32000)
    - MYEVS_N82_GAMMA_TRAJ0 (default 1.00)
    - MYEVS_N82_TRAJ_GATE_FLOOR (default 0.70)
    - MYEVS_N82_TRAJ_GATE_CEILING (default 1.40)
    - MYEVS_N82_TRAJ_CONF_K (default 0.25)
    - MYEVS_N82_TRAJ_CONF_FLOOR (default 0.15)
    - MYEVS_N82_G_TAU_SHORT_US (default 2000)
    - MYEVS_N82_G_TAU_LONG_US (default 20000)
    - MYEVS_N82_G_ALPHA (default 0.80)
    - MYEVS_N82_G_BETA (default 0.30)
    - MYEVS_N82_G_TRAJ_BETA (default 0.20)
    """
    ...
    tau0_us = int(_read_int_env("MYEVS_N82_TAU0_US", 16000))
    tau_inhib_us = int(_read_int_env("MYEVS_N82_TAU_INHIB_US", 32000))
    slow_ratio = float(_read_float_env("MYEVS_N82_SLOW_RATIO", 4.0))
    dt_max_ratio = float(_read_float_env("MYEVS_N82_FOOT_DT_MAX_RATIO", 8.0))
    ...
    rho0 = float(_read_float_env("MYEVS_N82_RHO0", 0.60))
    self_gain = float(_read_float_env("MYEVS_N82_SELF_GAIN", 0.55))
    lambda0 = float(_read_float_env("MYEVS_N82_LAMBDA0", 0.30))
    sigma_spatial = float(_read_float_env("MYEVS_N82_SIGMA_SPATIAL", 2.20))

    tau_traj_us = int(_read_int_env("MYEVS_N82_TAU_TRAJ_US", 12000))
    tau_traj_cons_us = int(_read_int_env("MYEVS_N82_TAU_TRAJ_CONS_US", 8000))
    traj_dt_max_us = int(_read_int_env("MYEVS_N82_TRAJ_DT_MAX_US", 32000))
    ...
    gamma_traj0 = float(_read_float_env("MYEVS_N82_GAMMA_TRAJ0", 1.00))
    traj_gate_floor = float(_read_float_env("MYEVS_N82_TRAJ_GATE_FLOOR", 0.70))
    traj_gate_ceiling = float(_read_float_env("MYEVS_N82_TRAJ_GATE_CEILING", 1.40))
    traj_conf_k = float(_read_float_env("MYEVS_N82_TRAJ_CONF_K", 0.25))
    traj_conf_floor = float(_read_float_env("MYEVS_N82_TRAJ_CONF_FLOOR", 0.15))

    g_tau_short_us = int(_read_int_env("MYEVS_N82_G_TAU_SHORT_US", 2000))
    g_tau_long_us = int(_read_int_env("MYEVS_N82_G_TAU_LONG_US", 20000))
    g_tau_short_ticks = float(tb.us_to_ticks(max(1, g_tau_short_us)))
    g_tau_long_ticks = float(tb.us_to_ticks(max(1, g_tau_long_us)))
    g_alpha = float(_read_float_env("MYEVS_N82_G_ALPHA", 0.80))
    g_beta = float(_read_float_env("MYEVS_N82_G_BETA", 0.30))
    g_traj_beta = float(_read_float_env("MYEVS_N82_G_TRAJ_BETA", 0.20))

    ker = _try_build_n82_kernel()
    scores = ker(
