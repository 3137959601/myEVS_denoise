...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n98 requires numba, but import failed")


def _try_build_n98_kernel():
    _require_numba()
...
def score_stream_n98(
    ...
    *,
    ...
    """N98: top3 2D (d, dt) compatibility aggregation backbone.

    Top3 supports are selected by minimal dt within same-polarity neighbors.
    Final score is the mean compatibility over available supports (1..3).
    """
    ...
    d0_px = float(_read_float_env("MYEVS_N98_D0_PX", 2.2))
    t0_us = float(_read_float_env("MYEVS_N98_T0_US", 1400.0))
    sigma_d_px = float(_read_float_env("MYEVS_N98_SIGMA_D_PX", 1.2))
    sigma_t_us = float(_read_float_env("MYEVS_N98_SIGMA_T_US", 5000.0))

    ker = _try_build_n98_kernel()
    ker(
