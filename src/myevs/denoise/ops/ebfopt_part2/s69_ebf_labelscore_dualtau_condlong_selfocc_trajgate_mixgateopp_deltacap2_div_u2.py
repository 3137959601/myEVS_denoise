...
def try_build_s69_ebf_labelscore_dualtau_condlong_selfocc_trajgate_mixgateopp_deltacap2_div_u2_scores_kernel():
    """Build and return Numba kernel for s69 score streaming.

    s69 is a controlled tweak on s63:
    - Keep s63's trajectory gate (centroid shift) and all states.
    - Relax the delta cap from `delta <= raw_short` to `delta <= 2*raw_short`.

    Hypothesis:
    - heavy seg1 recall is limited by overly conservative long-delta capping.
    - Allowing a bit more long-delta can boost signal scores without opening
      the floodgates as badly as removing the cap entirely.
