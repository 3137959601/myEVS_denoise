...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n81 requires numba, but import failed")


def _try_build_n81_kernel():
    _require_numba()
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            gate = traj_gate_floor + gamma_traj_eff * traj
            if gate < 0.0:
                gate = 0.0
            gate = np.power(gate, max(0.0, traj_gate_exp))

            score = (gate * e_dual) / (1.0 + lambda_eff * float(inhib_self[idx]))
...
def score_stream_n81(
    ...
    *,
    ...
    """N8.1: trajectory-gated dual-field model.

    score_i = gate(C_traj) * E_dual / (1 + lambda_eff * I_self)

    Env vars:
    - MYEVS_N81_TAU0_US (default 16000)
    - MYEVS_N81_TAU_INHIB_US (default 32000)
    - MYEVS_N81_SLOW_RATIO (default 4.0)
    - MYEVS_N81_FOOT_DT_MAX_RATIO (default 8.0)
    - MYEVS_N81_RHO0 (default 0.60)
    - MYEVS_N81_SELF_GAIN (default 0.55)
    - MYEVS_N81_LAMBDA0 (default 0.30)
    - MYEVS_N81_SIGMA_SPATIAL (default 2.20)
    - MYEVS_N81_TAU_TRAJ_US (default 12000)
    - MYEVS_N81_TAU_TRAJ_CONS_US (default 8000)
    - MYEVS_N81_TRAJ_DT_MAX_US (default 32000)
    - MYEVS_N81_GAMMA_TRAJ0 (default 1.20)
    - MYEVS_N81_TRAJ_GATE_FLOOR (default 0.25)
    - MYEVS_N81_TRAJ_GATE_EXP (default 1.0)
    - MYEVS_N81_G_TAU_SHORT_US (default 2000)
    - MYEVS_N81_G_TAU_LONG_US (default 20000)
    - MYEVS_N81_G_ALPHA (default 0.80)
    - MYEVS_N81_G_BETA (default 0.30)
    - MYEVS_N81_G_TRAJ_BETA (default 0.20)
    """
    ...
    tau0_us = int(_read_int_env("MYEVS_N81_TAU0_US", 16000))
    tau_inhib_us = int(_read_int_env("MYEVS_N81_TAU_INHIB_US", 32000))
    slow_ratio = float(_read_float_env("MYEVS_N81_SLOW_RATIO", 4.0))
    dt_max_ratio = float(_read_float_env("MYEVS_N81_FOOT_DT_MAX_RATIO", 8.0))
    ...
    rho0 = float(_read_float_env("MYEVS_N81_RHO0", 0.60))
    self_gain = float(_read_float_env("MYEVS_N81_SELF_GAIN", 0.55))
    lambda0 = float(_read_float_env("MYEVS_N81_LAMBDA0", 0.30))
    sigma_spatial = float(_read_float_env("MYEVS_N81_SIGMA_SPATIAL", 2.20))

    tau_traj_us = int(_read_int_env("MYEVS_N81_TAU_TRAJ_US", 12000))
    tau_traj_cons_us = int(_read_int_env("MYEVS_N81_TAU_TRAJ_CONS_US", 8000))
    traj_dt_max_us = int(_read_int_env("MYEVS_N81_TRAJ_DT_MAX_US", 32000))
    ...
    gamma_traj0 = float(_read_float_env("MYEVS_N81_GAMMA_TRAJ0", 1.20))
    traj_gate_floor = float(_read_float_env("MYEVS_N81_TRAJ_GATE_FLOOR", 0.25))
    traj_gate_exp = float(_read_float_env("MYEVS_N81_TRAJ_GATE_EXP", 1.0))

    g_tau_short_us = int(_read_int_env("MYEVS_N81_G_TAU_SHORT_US", 2000))
    g_tau_long_us = int(_read_int_env("MYEVS_N81_G_TAU_LONG_US", 20000))
    g_tau_short_ticks = float(tb.us_to_ticks(max(1, g_tau_short_us)))
    g_tau_long_ticks = float(tb.us_to_ticks(max(1, g_tau_long_us)))
    g_alpha = float(_read_float_env("MYEVS_N81_G_ALPHA", 0.80))
    g_beta = float(_read_float_env("MYEVS_N81_G_BETA", 0.30))
    g_traj_beta = float(_read_float_env("MYEVS_N81_G_TRAJ_BETA", 0.20))

    ker = _try_build_n81_kernel()
    scores = ker(
