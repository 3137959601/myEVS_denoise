...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n161 requires numba, but import failed")
...
def _try_build_n161_kernel():
    ...
    def _kernel(
        ...
        for i in range(n):
            ...
            # Non-S52 polarity reformulation (linear variant):
            # - keep same-polarity evidence as main carrier.
            # - apply a mild linear penalty for opposite-polarity mixing.
            # - retain support-based confidence gain and moderate center suppression.
            etot = raw_same + raw_opp
            if etot <= 0.0:
                out[i] = np.float32(0.0)
                continue

            mix = raw_opp / (etot + eps)
            if mix < 0.0:
                mix = 0.0
            if mix > 1.0:
                mix = 1.0

            pol_linear = 1.0 - 0.5 * mix
            if pol_linear < 0.5:
                pol_linear = 0.5
            if pol_linear > 1.0:
                pol_linear = 1.0

            support_gain = 1.0 + 0.5 * sfrac
            u_eff = u_lite * (1.0 - 0.35 * sfrac)
            if u_eff < 0.0:
                u_eff = 0.0
            if u_eff > 1.0:
                u_eff = 1.0

            out[i] = np.float32((raw_same * pol_linear * support_gain) / (1.0 + u_eff * u_eff))

    return _kernel


def score_stream_n161(
    ev,
    *,
    width: int,
    height: int,
    ...
    """N161: compact-lut backbone + linear polarity reformulation (non-S52).
