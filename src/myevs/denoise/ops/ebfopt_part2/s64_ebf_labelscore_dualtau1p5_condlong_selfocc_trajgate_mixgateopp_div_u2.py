...
def try_build_s64_ebf_labelscore_dualtau1p5_condlong_selfocc_trajgate_mixgateopp_div_u2_scores_kernel():
    """Build and return Numba kernel for s64 score streaming.

    s64 is a diagnostic follow-up to s63 (7.16 priority-2):
    - Keep the same trajectory proxy gate as s63 (centroid shift between
      short/long same-pol support).
    - Reduce the long window span from 2*tau to ~1.5*tau:
        tau_long = tau + tau//2
      This tests whether "long boost" in heavy is mainly harmed by integrating
      too much background/hot noise when the span is too large.

    No new sweep hyperparameters; still streaming and O(r^2).
    """
