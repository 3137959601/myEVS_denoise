...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n151 requires numba, but import failed")
...
def _try_build_n151_kernel():
    ...
    def _kernel(
        ...
        prev_dt_q = np.zeros((npx,), dtype=np.uint16)
        ...
        t0 = int(float(tau_base) * t0_ratio + 0.5)
        td = int(float(tau_base) * td_ratio + 0.5)
        if t0 <= 1:
            t0 = 1
        if td <= 1:
            td = 1
        ...
        for i in range(n):
            ...
            same_cont = (ts0 != 0 and pol0 == pi and ti > ts0)
            if same_cont:
                dt0 = ti - ts0
                if dt0 < 0:
                    dt0 = -dt0
            ...
            if d01 < 0:
                d01 = -d01
            u_rec = np.exp(-float(dt0) * inv_t0) * np.exp(-float(d01) * inv_td)
            if u_rec < 0.0:
                u_rec = 0.0
            if u_rec > 1.0:
                u_rec = 1.0
            ...
            base_score = raw_gated / (1.0 + (u_rec * u_rec))
...
def score_stream_n151(
    ...
    *,
    ...
    """N151: n150 with stronger center recurrence proxy (dt0 + dt1)."""
    ...
    sigma_space = float(_read_float_env("MYEVS_N151_SIGMA", 2.5))
    beta_init = float(_read_float_env("MYEVS_N151_BETA_INIT", 0.5))
    t0_ratio = float(_read_float_env("MYEVS_N151_T0_RATIO", 0.5))
    td_ratio = float(_read_float_env("MYEVS_N151_TD_RATIO", 0.25))
    if sigma_space <= 1e-6:
        sigma_space = 1e-6
    if t0_ratio <= 0.0:
        t0_ratio = 0.5
