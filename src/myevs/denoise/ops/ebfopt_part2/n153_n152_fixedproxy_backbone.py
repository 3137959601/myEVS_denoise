...
# Fixed constants from n152-b (2026-04-22), kept internal to avoid extra tuning knobs.
N153_SIGMA = 2.8
N153_BETA_INIT = 0.65
N153_K_SFRAC = 0.65
N153_K_MIX = 0.20
...
def _require_numba() -> None:
    if numba is None:
        raise RuntimeError("n153 requires numba, but import failed")
...
def score_stream_n153(
    ...
    *,
    ...
    """N153: fixed-proxy n152 (zero new user hyperparameters).

    Key changes relative to n152:
    - Keeps the same scoring form and state footprint.
    - Freezes center-proxy constants to remove user-side extra tuning knobs.
    """
    ...
    sigma_space = float(N153_SIGMA)
    beta_init = float(N153_BETA_INIT)
    k_sfrac = float(N153_K_SFRAC)
    k_mix = float(N153_K_MIX)
    if sigma_space <= 1e-6:
        sigma_space = 1e-6
