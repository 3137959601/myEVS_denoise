...
| `STCF_orig` | `cpp` | 0.8830 | 0.8086 | stcf_orig_k3 | 重跑 |
...
|---|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|---|:---:|
| 1hz | **v2.2** | **0.9512** | EBF | 0.9484 | YNoise | 0.9408 | N149 | 0.9381 | TS | 0.9298 | BAF | 0.9166 | PFD | 0.9135 | STCF_orig | 0.9102 | MLPF | 0.8632 | EvFlow | 0.8486 | KNoise | 0.6359 |
| 3hz | **v2.2** | **0.9502** | EBF | 0.9444 | N149 | 0.9394 | YNoise | 0.9361 | TS | 0.9279 |MLPF | 0.9238| PFD | 0.9079 | STCF_orig | 0.9075 | BAF | 0.8919 | EvFlow | 0.8424 | KNoise | 0.6232 | | |
| 5hz | **v2.2** | **0.9500** | N149 | 0.9416 | EBF | 0.9408 | YNoise | 0.9312 | TS | 0.9259 | MLPF | 0.9012 | PFD | 0.8984 | STCF_orig | 0.8965 | BAF | 0.8651 | EvFlow | 0.8206 | KNoise | 0.6239 |
| 7hz | **v2.2** | **0.9475** | EBF | 0.9387 | N149 | 0.9381 | MLPF | 0.9238 | YNoise | 0.9283 | TS | 0.8945 | STCF_orig | 0.8927 | PFD | 0.8916 | BAF | 0.7657 | EvFlow | 0.7219 | KNoise | 0.6170 | | |
| 10hz | **v2.2** | **0.9462** | EBF | 0.9346 | YNoise | 0.9227 | MLPF | 0.9199 | TS | 0.8993 | STCF_orig | 0.8830 | PFD | 0.8794 | BAF | 0.8269 | EvFlow | 0.7006 | KNoise | 0.6172 |N149|~|| | |
...
| **N149_v2.2** | 0.9563 | — | 0.9443 | 0.9375 | sigma=2.75 alpha=0.25 |
| `STCF` | 0.9460 | 0.9047 | 0.8962 | 0.8791 | 2.1 补跑 |
| `EBF` | 0.9416 | 0.9061 | 0.9185 | 0.9099 | 2.1 补跑 |
| `YNoise` | 0.9227 | 0.8782 | 0.9083 | 0.8971 | 2.1 补跑 |
| `MLPF` | 0.8669 | 0.8966 | 0.8140 | 0.7729 | patch=7/自训练（四档完成） |
| `BAF` | 0.9013 | 0.7614 | 0.8417 | 0.8165 | r=1 fixed |
| `PFD` | 0.8030 | 0.7972 | 0.7932 | 0.7856 | |
| `STCF_orig` | 0.9013 | 0.8217 | 0.8598 | 0.8402 | |
| `TS` | 0.8619 | 0.7640 | 0.8528 | 0.8465 | 2.1 补跑 |
| `EvFlow` | 0.8351 | 0.7154 | 0.8022 | 0.7847 | 2.1 补跑 |
...
| `EBF` | 0.9681 | 0.9621 | 0.9422 | 0.9426 | (5,512ms) | (5,512ms) | (5,512ms) | (5,512ms) | |
| `YNoise` | 0.9601 | 0.9417 | 0.7859 | 0.9274 | (5,256ms) | (5,256ms) | (5,256ms) | (5,256ms) | |
| `MLPF` | 0.9380 | 0.9356 | 0.8493 | 0.9217 | (2,32ms) | (2,128ms) | (2,32ms) | (2,32ms) | patch=5/自训练（四档完成） |
...
| `BAF` | 0.9489 | 0.9352 | 0.9149 | 0.8960 | (1,512ms) | (1,512ms) | (1,512ms) | (1,512ms) | r=1 fixed 重跑 |
| `EvFlow` | 0.8877 | 0.8894 | 0.8717 | 0.7939 | (6,32ms) | (4,32ms) | (2,32ms) | (2,32ms) | |
| `TS` | 0.9102 | 0.9090 | 0.8977 | 0.8819 | (4,128ms) | (4,128ms) | (4,128ms) | (4,128ms) | |
...
| `PFD` | `cpp` | 0.6940 | — | (1, 8ms) | r=1 fixed (10场景均值, 2026-05-21 重跑) |
| `MLPF` | `python` | 0.9823 | 0.9763 | (3, 64ms) | 10场景逐场景训练+评测完成 |
...
| **N149_v2.2** | `cpp` | **0.8872** | — | (2,8K) | sigma=2.0 alpha=2.0, 10场景均值 |
| `STCF_orig` | `cpp` | 0.8633 | — | (1, 16ms, k=2) | 10场景均值, 补跑 |
| `EBF` | `cpp` | 0.8338 | — | (2, 16ms) | 10场景均值, 补跑 |
| `YNoise` | `cpp` | 0.8314 | — | (2, 8ms) | 10场景均值, 补跑 |
| `PFD` | `cpp` | 0.7847 | — | (1, 8ms) | r=1 fixed (10场景均值, 补跑) |
| `MLPF` | `python` | 0.6711 | 0.5703 | (3, 16ms) | 10场景逐场景训练+评测完成（patch=7） |
| `STCF` | `cpp` | 0.7911 | — | (2, 4ms) | 10场景均值, 补跑 |
| `BAF` | `cpp` | 0.7488 | — | (1, 2ms) | 10场景均值, 补跑 |
| `TS` | `cpp` | 0.7020 | — | (2, 8ms) | 10场景均值, 补跑 |
| `KNoise` | `cpp` | 0.5276 | — | (1, 2ms) | 10场景均值, 补跑 |

> LED 10 场景（scene_100~1046）全部补跑。N149 v2.2 明细：100=0.9262, 1004=0.8588, 1018=0.8889, 1028=0.8993, 1032=0.8954, 1033=0.8650, 1034=0.8776, 1043=0.8913, 1045=0.8773, 1046=0.8917。

> 所有 LED 已改为 10 场景均值，上表同。旧 scene_1004 单表移除。
...
### 7.12 对比算法边界检测与修复策略 (2026-05-22，待执行)

> **问题**：非 N149 对比算法（BAF/STCF/EBF/KNoise/YNoise/TS/EvFlow/PFD/MLPF）在 ED24、DVSCLEAN、LED 上使用固定参数运行，未做参数扫频。部分算法的最优阈值触及扫频边界（0 或 5，当r>5时不继续增加了，但是标注达到边界），或算法参数（r/tau）未经调优，导致 AUC 偏低。
...
| KNoise | tau 扫 | 触边界则扩展 |
| MLPF | 3hz 重训练 + 评估 | patch=7, fulltrain |

**执行**：`scripts/n149_ablation/run_boundary_fix.py`（待写），多线程。MLPF 3hz 单独 `scripts/train_mlpf_torch.py`。
