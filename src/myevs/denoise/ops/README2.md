...
| `PFD` | `cpp` | 0.9135 | 0.9579 | pfd_r3_tau16000_m2 | |
| `STCF_orig` | `cpp` | 0.9102 | 0.9502 | stcfo_1hz_tau16ms_k2 | |
| `EvFlow` | `cpp` | 0.8486 | 0.9501 | evflow_r2_tau16000 | |
| `MLPF*` | `python` | 0.8391 | 0.9518 | mlpf_fulltrain_patch7 | |
...
| `MLPF` | `python` | 0.8772 | 0.9343 | mlpf_p9_2hz | |
...
| `PFD` | `cpp` | 0.9079 | 0.9214 | pfd_r3_tau8000_m2 | |
...
| `MLPF` | `python` | 0.9012 | 0.8910 | mlpf_p9_5hz | |
| `STCF_orig` | `cpp` | 0.8965 | 0.8428 | stcfo_5hz_tau16ms_k4 | |
| `PFD` | `cpp` | 0.8984 | 0.8850 | pfd_r3_tau8000_m2 | |
...
| `MLPF` | `python` | 0.9061 | 0.8591 | mlpf_p9_8hz | |
| `STCF_orig` | `cpp` | 0.8883 | 0.7467 | stcfo_8hz_tau16ms_k5 | |
| `PFD` | `cpp` | 0.8825 | 0.8132 | pfd_r3_tau8000_m2 | |
| `BAF` | `cpp` | 0.8379 | 0.7806 | baf_r1 | |
| `KNoise` | `cpp` | 0.6214 | — | knoise_tau8000 | |

### 7.6 MLPF C++ vs Python 对比 (Driving-ED24)

| Level | C++ AUC | Python AUC | Δ AUC | 备注 |
|---|---:|---:|---:|---|
| 1hz | **0.9235** | 0.8632 | +0.0603 | C++ per-threshold fresh state |
| 2hz | **0.9252** | 0.8772 | +0.0480 | |
| 5hz | **0.9234** | 0.9012 | +0.0222 | |
| 8hz | **0.9218** | 0.9061 | +0.0157 | |
...
| Method | light AUC | mid AUC | heavy AUC | Best (r,tau) | 备注 |
|---|---:|---:|---:|---|---|
...
| `BAF` | 0.9119 | 0.8391 | 0.8161 | (2,64ms) | |
| `PFD` | 0.8966 | 0.8889 | 0.8757 | (3,32ms,m=1) | |
| `STCF_orig` | 0.8664 | 0.8415 | 0.8277 | (1,64ms,k=1-2) | |
| `TS` | 0.8619 | 0.8528 | 0.8465 | (2,32ms) | |
| `EvFlow` | 0.8351 | 0.8022 | 0.7847 | (4,32ms) | |
| `KNoise` | 0.7130 | 0.6625 | 0.6417 | (1,32ms) | |
...
| `STCF_orig` | 0.9298 | 0.9245 | 0.9165 | (1,64ms,k=1) | |
| `BAF` | 0.9265 | 0.8780 | 0.8210 | (4,64ms) | |
| `EvFlow` | 0.9127 | 0.8091 | 0.5880 | (5,64ms) | |
...
| `STCF_orig` | `cpp` | 0.9810 | — | (1, 32ms, k=3-4) | |
...
2. **α 不会降低精度——针对场景选择正确的 α 反而提升性能**。Driving 设 α=0 比默认 EMA 提升 +0.006~+0.015；LED 设 α=1.0 提升 +0.002~+0.008；Ped/Bike 设 α=0.25 等价于默认 EMA（|Δ| < 0.002）。

3. **α 的取值反映了数据集的客观物理机制——"极性信息含量"**。Driving 为 v2e 仿真叠加，异极事件是噪声 → α=0（仅同极有效）；LED 为稀疏亮斑，异极同样是稀疏信号 → α=1.0（全极性等权）；Ped/Bike 为真实 DVS 拍摄，异极有弱相关性但不完全可信 → α≈0.25（轻度打折）。α 的取值从 0 到 1 完美映射了极性从"完全不可信"到"完全可信"的连续谱。

4. **论文可作为"自适应极性置信度"讨论**：α 不需要设为自由超参数扫频——根据传感器类型和数据特征即可先验确定：仿真叠加数据取 0，真实 DVS 数据取 0.25，稀疏信号取 1.0。三个值覆盖当前所有场景，且均有实验和物理直觉双重支撑。

### 10.5 结论

1. **N149 v2.1 在所有 11 个场景上无退化**
2. beta/sfrac 移除，h 简化为单行，wt 平方保留
3. α 可进一步简化：**Driving 场景用 instant mix 或 α=0，Ped/Bike 用 EMA 或 α=0.25**——场景自适应或超参数两个方向均可行
4. **即日起 N149 v2.1 为正式算法**
4. v2 公式更简洁：3 项计算（同极证据、异极证据×抑制、自激励抑制）→ 1 个分数，不再有 `1+b_i s_i` 调制因子
5. 旧 N149（含 beta/sfrac）作为历史版本保留，不再用于新实验
