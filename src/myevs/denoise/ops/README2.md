...
| **N149_v2.2** | 0.9561 | — | 0.9346 | (5,256ms) | — | (5,256ms) | **v2.2** alpha=0.25 |
...
| **N149_v2.2** | 0.9840 | 0.9815 | 0.9763 | — | (5,512ms) | (5,512ms) | (5,512ms) | — | **v2.2** alpha=0.25 |
...
| `EvFlow` | 0.9127 | 0.8091 | 0.5880 | — | (5,64ms) | (5,64ms) | (5,64ms) | — | |
...
$$
\boxed{S_i=\widetilde{R}_i\cdot\frac{h_i+T_r}{2h_i+T_r}
       =\big(R_i^{+}+(1-m_i)^2R_i^{-}\big)\cdot\frac{h_i+T_r}{2h_i+T_r}}
$$

> \(T_r = \tau \cdot u\_denom\_factor\)（默认 1.0）。\(T_r\) 可与邻域证据的 \(\tau\) 解耦——长 τ 场景设 \(T_r = \tau/4\) 可在低 bit 宽度下恢复折扣动态范围（§8.16）。

**热状态更新**（每事件，在邻域证据采集之前）：

$$h_i \leftarrow \max(0,\ h_i + T_r - 2\Delta t_i)$$

> 单行无分支。物理含义：每事件净增 \(T_r\)，时间以 2 倍速率侵蚀。与原始两段式 `max(0,h-Δt)+max(0,Tr-Δt)` 等价（§8.17 实验验证）。\(h_i\) 截断至 N bit（饱和，非回绕），FPGA 部署通常 12-16 bit（§9）。
...
**v2.2 公式**（仅 α 变更，其余同 v2.1）：

$$S_i = (R_i^{+} + \alpha \cdot R_i^{-}) \cdot \frac{h_i+T_r}{2h_i+T_r}$$
...
**版本演进**：原始(5状态) → v2(-b,s) → v2.1(简化h,合并分母) → **v2.2(α固定先验,去EMA)**
...
## 11. N149 v2.2 超参数消融实验 (Phase 1 完成)

### 11.1 数据集与默认最优参数

| 数据集 | 级别数 | 默认 r | 默认 tau | 默认 sigma | 默认 alpha | Tr 策略 |
|---|---|---|---|---|---|---|
| Driving-ED24 | 5 (1/3/5/7/10hz) | 2 | 32K | 3.0 | 0 | tau |
| ED24 Pedestrian | 4 (1.8/2.1/2.5/3.3) | 5 | 256K | 3.0 | 0.25/0.2 | tau/8 |
| ED24 Bicycle | 4 (1.8/2.1/2.5/3.3) | 5 | 512K | 3.0 | 0.25/0.2 | tau/8 |
| DVSCLEAN | 10 (5 scenes × 2 ratios) | 5 | 128K | 3.0 | 0.25 | tau |
| LED | 10 (scene_100~1046) | 2 | 16K | 3.0 | 1.0 | tau |

### 11.2 扫频范围

| 超参数 | Driving | ED24 Ped/Bike | DVSCLEAN | LED |
|---|---|---|---|---|
| r | {1,2,3,4,5,7} | {1,2,3,5,7,9} | {1,2,3,5,7,9} | {1,2,3,5,7} |
| tau (us) | {4K,8K,16K,32K,64K,128K,256K} | {32K,64K,128K,256K,384K,512K} + Bike{768K} | {16K,32K,64K,128K,256K,512K} | {2K,4K,8K,16K,32K,64K,128K} |
| sigma | {1.0~7.0, 8点} | 同左 | 同左 | 同左 |
| alpha | {0,0.05,0.1,0.25,0.5,0.75,1.0,ema} | 同左 + 细调 | 同左 | {0,0.25,0.5,0.75,1.0,1.5,2.0,ema} |

### 11.3 脚本与数据

**脚本**：`scripts/n149_ablation/run_phase1_{drive,ped,bike,dvsclean,led}.py`（8 线程，控制台实时进度 + 边界检测）

**数据**：`data/Hyperparameter ablation_study/{drive,ped,bike,dvsclean,led}/phase1_{dataset}.csv`

**运行**（5 终端并行）：
```bash
cd scripts/n149_ablation
python run_phase1_drive.py
python run_phase1_ped.py
python run_phase1_bike.py
python run_phase1_dvsclean.py
python run_phase1_led.py
```

### 11.4 Phase 1 最终结果 (2026-05-21)

全部 5 数据集（33 级别）粗调扫频完成。**"跨级一致值" = 多数级别的最优值；有分歧时标注少数派。**

**总览**：

| 数据集 | 最优 r | r=2 损失 | 最优 tau | 最优 sigma | 最优 alpha | 备注 |
|---|---|---|---|---|---|---|
| **Driving** | **2** | 0 | 32K (1hz 偏 64K) | 1.5 (1hz 偏 2.0) | **0** | — |
| **ED24 Ped** | 5 (r≥7 饱和) | **-0.016** | 256K (3.3 偏 128K) | 2.5-3.0 | **0.2** | r=9 仅 +0.001 |
| **ED24 Bike** | **5** | -0.007 | **256K** | **2.5** | **0.2** | 4 级完全一致 |
| **DVSCLEAN** | 5 (7 场景偏 9) | — | 128K (3 场景偏 64K) | 2.5 | 0.25 | r=9 仅 +0.001 |
| **LED** | **2** | 0 | 8K (4 场景偏 16K) | 1.5 | **1.0** | — |

> r=2 损失 = AUC(r=2) - AUC(r=5)。Ped 损失 0.016 最大。Ped/DVSCLEAN 的 r=9 比 r=5 高 <0.0011，r=5 为实际甜点。
...
|---|---|---|---|---|---|---|
| r | 2 | 2 | 2 | 2 | 2 | **2** |
| tau | 64K | 32K | 32K | 32K | 32K | **32K** (1hz 偏 64K) |
| sigma | 2.0 | 1.5 | 1.5 | 1.5 | 1.5 | **1.5** (1hz 偏 2.0) |
| alpha | 0 | 0 | 0.05 | 0.05 | 0.1 | **0** |
...
|---|---|---|---|---|---|
| r | 9* | 9* | 9* | 9* | **5** (r≥5 已饱和) |
| tau | 256K | 256K | 256K | 128K | **256K** (3.3 偏 128K) |
| sigma | 3.0 | 2.5 | 2.5 | 2.5 | **2.5** (1.8 偏 3.0) |
| alpha (细) | 0.25 | 0.20 | 0.25 | 0.25 | **0.2** (0.15-0.25 等价) |
...
|---|---|---|---|---|---|
| r | 5 | 5 | 5 | 5 | **5** |
| tau | 256K | 256K | 256K | 256K | **256K** |
| sigma | 2.5 | 2.5 | 2.5 | 2.5 | **2.5** |
| alpha (细) | 0.15 | 0.15 | 0.20 | 0.20 | **0.2** (0.15-0.2 等价) |

> ED24 全 7 级 (Ped+Bike) 统一 α=0.2，距各最优 ≤0.0003。Bike 3.3 为新增数据，与其他 3 级完全一致（r=5, tau=256K, sigma=2.5, alpha=0.2）。

**DVSCLEAN（10 场景）**：r: 3 场景偏 5 / 7 场景偏 9；tau: 3 场景偏 64K / 7 场景偏 128K；sigma: 6 场景偏 2.5 / 4 场景偏 3.0；alpha: 9/10 场景偏 0.25。

**LED（10 场景）**：r=2 为主（8/10），tau=8K（6/10）vs 16K（4/10），sigma=1.5（6/10）vs 2.0（4/10），alpha=1.0 全 10 场景一致。

### 11.5 Phase 2 计划

1. **Ped/DVSCLEAN r 边界**：扫 {11,13,15} 确认 r=5 后无显著提升
2. **sigma 细调**：最优值 ±0.5 范围，0.25 步长
3. **绘图**：Phase 2 完成后 4-panel 学术风格折线图
