...
Output: data/n149_ablation/phase1_ped.csv
"""
...
OUT = r"D:/hjx_workspace/scientific_reserach/projects/myEVS/data/Hyperparameter ablation_study/ped"
...
W, H = 346, 260
R_DEF, TAU_DEF, SIGMA_DEF, ALPHA_DEF = 5, 256000, 3.0, "0.25"

SWEEPS = {
    "r": [1,2,3,5,7,9],
    "tau":   [32000, 64000, 128000, 256000, 384000, 512000],
...
def run(level, clean, noisy, param, value):
    ...
    tag = "p1_ped_%s_%s_%s" % (level, param, str(value).replace("/","_").replace(".","p"))
    csv = "%s/%s.csv" % (OUT, tag)
...
total = len(tasks)
print("[%s] ED24 Pedestrian Phase1: %d levels x %d param-sweeps = %d tasks" % (
...
df.to_csv("%s/phase1_ped.csv" % OUT, index=False)
