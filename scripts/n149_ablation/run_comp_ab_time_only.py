"""Add time_only (= no_spatial + no_hot + blind) to component ablation. 8 threads."""
...
def run_one(entry):
    ...
    csv = f"{OUT}/{ds}/_comp_ab/comp_ab_{ds}_{lv}_time_only.csv"
    try:
        ...
        cmd = [PY, '-m', 'myevs.cli', 'roc',
               ...
               '--tag', f'comp_ab_{ds}_{lv}_time', '--out-csv', csv, '--append']
...
total = len(DATASETS)
print(f"time_only (no_spatial+no_hot+blind): {total} tasks, 8 threads")
t0 = time.time()
results, errors = [], []
...
if results:
    ...
    df.to_csv(f"{OUT}/component_ablation_time_only.csv", index=False)
    ...
    print(f"\n=== time_only vs baseline ===")
    for ds in ["drive","ped","bike","dvsclean","led"]:
        sub = merged[merged.dataset==ds]
        if len(sub)==0: continue
        print(f"  {ds}: time_only AUC={sub.auc.mean():.4f}  baseline={sub.baseline_auc.mean():.4f}  Δ={sub.delta.mean():+.4f}")
if errors:
