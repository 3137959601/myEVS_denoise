...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N126_ALPHA"] = str(float(alpha))
    env["MYEVS_N126_TAU1_US"] = str(int(tau1_us))
    env["MYEVS_N126_RAW_THR"] = str(float(raw_thr))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"sweep failed for alpha={alpha}, tau1_us={tau1_us}, raw_thr={raw_thr}: return code {proc.returncode}"
        )

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n126_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n126 top1-bonus params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n126_top1_bonus")

    ap.add_argument("--alpha-grid", default="0.05,0.1,0.2,0.3")
    ap.add_argument("--tau1-us-grid", default="2000,5000,10000,20000")
    ...
    for alpha in alpha_grid:
        for tau1_us in tau1_grid:
            for raw_thr in raw_thr_grid:
                name = f"a{fmt_f(alpha)}_t{int(tau1_us)}_r{fmt_f(raw_thr)}"
