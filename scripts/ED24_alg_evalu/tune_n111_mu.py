...
def parse_float_list(s: str) -> list[float]:
    ...
    for part in str(s).split(","):
        ...
        out.append(float(p))
    if not out:
        raise ValueError("empty mu list")
    return out


def fmt_mu(v: float) -> str:
...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N111_MU"] = str(float(mu))
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"sweep failed for mu={mu}: return code {proc.returncode}")

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n111_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n111 MU via slim sweep and aggregate metrics.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n111_mu")

    ap.add_argument("--mu-grid", default="0.0,0.5,1.0,1.5,2.0,3.0")
    ...
    mu_grid = parse_float_list(args.mu_grid)
