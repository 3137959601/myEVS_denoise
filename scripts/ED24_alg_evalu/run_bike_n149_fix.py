"""Re-run N149 v2.2 on ED24 Bicycle: r×tau sweep, sigma=2.75, alpha=0.25. 12-threaded."""
...
W, H = 346, 260
R_LIST = [3, 4, 5]
TAU_LIST = [32000, 64000, 128000, 256000]

def run_one(lv, clean, noisy, r, tau):
    csv = f"{OUT}/_n149_cache/n149_{lv}_r{r}_tau{tau}.csv"
    ...
    cmd = [PY, '-m', 'myevs.cli', 'roc',
           ...
           '--tag', f'n149_fix_r{r}_tau{tau}', '--out-csv', csv, '--append']
...
total = len(tasks)
print(f"N149 Bike: {total} tasks ({len(LEVELS)}lv x {len(R_LIST)}r x {len(TAU_LIST)}tau)")
t0 = time.time()
...
df.to_csv(f"{OUT}/n149_bike_v2.csv", index=False)

print("\n=== N149 BIKE PER-LEVEL BEST ===")
