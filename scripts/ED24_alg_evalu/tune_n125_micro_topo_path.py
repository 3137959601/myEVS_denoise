...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N125_PATH_DEPTH"] = str(int(depth))
    env["MYEVS_N125_TAU_STEP_US"] = str(int(tau_step_us))
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"sweep failed for depth={depth}, tau_step_us={tau_step_us}: return code {proc.returncode}")

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n125_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n125 micro-topological path params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n125_micro_topo_path")

    ap.add_argument("--path-depth-grid", default="2,3,4")
    ap.add_argument("--tau-step-us-grid", default="2000,5000,10000")
    ...
    depth_grid = parse_int_list(args.path_depth_grid)
    tau_step_grid = parse_int_list(args.tau_step_us_grid)

    results: list[dict] = []
    for depth in depth_grid:
        for tau_step_us in tau_step_grid:
            name = f"d{int(depth)}_step{int(tau_step_us)}"
    ...
    if args.full_validate:
        ...
        full = run_one(
            ...
            project_root=project_root,
            out_dir=out_dir,
            depth=int(best["path_depth"]),
            tau_step_us=int(best["tau_step_us"]),
