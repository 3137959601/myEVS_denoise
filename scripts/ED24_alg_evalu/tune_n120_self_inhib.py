...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N120_TAU_SELF_US"] = str(int(tau_self_us))
    env["MYEVS_N120_R_MAX"] = str(float(r_max))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"sweep failed for tau_self_us={tau_self_us}, r_max={r_max}: return code {proc.returncode}"
        )

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n120_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n120 self-inhibition params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n120_self_inhib")

    ap.add_argument("--tau-self-us-grid", default="10000,30000,50000")
    ap.add_argument("--r-max-grid", default="2.0,3.0,4.0,5.0")
    ...
    tau_self_grid = parse_int_list(args.tau_self_us_grid)
    r_max_grid = parse_float_list(args.r_max_grid)

    results: list[dict] = []
    for tau_self_us in tau_self_grid:
        for r_max in r_max_grid:
            name = f"tself{int(tau_self_us)}_r{fmt_f(float(r_max))}"
    ...
    if args.full_validate:
        ...
        full = run_one(
            ...
            project_root=project_root,
            out_dir=out_dir,
            tau_self_us=int(best["tau_self_us"]),
