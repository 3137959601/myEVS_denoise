...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N124_LAMBDA"] = str(float(lam))
    ...
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n124_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n124 synergy tri-filter params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n124_synergy_trifilter")

    ap.add_argument("--lambda-grid", default="0.0,0.3,0.5,0.8,1.0")
