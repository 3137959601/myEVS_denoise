...
# 输出：data/ED24/myPedestrain_06/EBF/roc_ebf_{light,mid,heavy}_label_exact.csv/.png
...
# ED24 myPedestrain_06 数据路径（1.8/2.5/3.3 分别映射到 light/mid/heavy）
...
$OUT = "data/ED24/myPedestrain_06/EBF/roc_ebf_light_label_exact.csv"; [System.IO.File]::WriteAllText($OUT, "")
...
& $PY -m myevs.cli plot-csv --in $OUT --out "data/ED24/myPedestrain_06/EBF/roc_ebf_light_label_exact.png" --x fpr --y tpr --group tag --kind line --xlabel FPR --ylabel TPR --title "EBF ROC (light, label exact)"
Show-TopAuc -CsvPath $OUT -TopN 10

Write-Host "=== EBF (ebf) slomo: mid (sweep r/tau) ==="
$OUT = "data/ED24/myPedestrain_06/EBF/roc_ebf_mid_label_exact.csv"; [System.IO.File]::WriteAllText($OUT, "")
foreach ($r in $EBF_RADIUS_LIST) {
	foreach ($tau in $EBF_TAU_LIST) {
...
& $PY -m myevs.cli plot-csv --in $OUT --out "data/ED24/myPedestrain_06/EBF/roc_ebf_mid_label_exact.png" --x fpr --y tpr --group tag --kind line --xlabel FPR --ylabel TPR --title "EBF ROC (mid, label exact)"
Show-TopAuc -CsvPath $OUT -TopN 10

Write-Host "=== EBF (ebf) slomo: heavy (sweep r/tau) ==="
$OUT = "data/ED24/myPedestrain_06/EBF/roc_ebf_heavy_label_exact.csv"; [System.IO.File]::WriteAllText($OUT, "")
foreach ($r in $EBF_RADIUS_LIST) {
	foreach ($tau in $EBF_TAU_LIST) {
...
& $PY -m myevs.cli plot-csv --in $OUT --out "data/ED24/myPedestrain_06/EBF/roc_ebf_heavy_label_exact.png" --x fpr --y tpr --group tag --kind line --xlabel FPR --ylabel TPR --title "EBF ROC (heavy, label exact)"
Show-TopAuc -CsvPath $OUT -TopN 10
