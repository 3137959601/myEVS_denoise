...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N116_TAU_BURST_US"] = str(int(tau_burst_us))
    env["MYEVS_N116_THRESH"] = str(int(thresh))
    env["MYEVS_N116_PENALTY"] = str(float(penalty))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"sweep failed for tau_burst_us={tau_burst_us}, thresh={thresh}, penalty={penalty}: return code {proc.returncode}"
        )

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n116_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n116 burst gate params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n116_burst")

    ap.add_argument("--tau-burst-us-grid", default="1000,2000,4000")
    ap.add_argument("--thresh-grid", default="1,2,3")
    ap.add_argument("--penalty-grid", default="0.0,0.2,0.5")
    ...
    tau_burst_us_grid = parse_int_list(args.tau_burst_us_grid)
    thresh_grid = parse_int_list(args.thresh_grid)
    penalty_grid = parse_float_list(args.penalty_grid)

    results: list[dict] = []
    for tau_burst_us in tau_burst_us_grid:
        for thresh in thresh_grid:
            for penalty in penalty_grid:
                name = f"tb{int(tau_burst_us)}_th{int(thresh)}_p{fmt_f(float(penalty))}"
    ...
    if args.full_validate:
        ...
        full = run_one(
            ...
            project_root=project_root,
            out_dir=out_dir,
            tau_burst_us=int(best["tau_burst_us"]),
