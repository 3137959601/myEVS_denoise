...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N121_R_CORE"] = str(int(r_core))
    env["MYEVS_N121_BETA"] = str(float(beta))
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"sweep failed for r_core={r_core}, beta={beta}: return code {proc.returncode}")

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n121_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n121 center-surround params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n121_center_surround")

    ap.add_argument("--r-core-grid", default="1")
    ap.add_argument("--beta-grid", default="0.1,0.2,0.3,0.5")
    ...
    r_core_grid = parse_int_list(args.r_core_grid)
    beta_grid = parse_float_list(args.beta_grid)

    results: list[dict] = []
    for r_core in r_core_grid:
        for beta in beta_grid:
            name = f"rcore{int(r_core)}_b{fmt_f(float(beta))}"
