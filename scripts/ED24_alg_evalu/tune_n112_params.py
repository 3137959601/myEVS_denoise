...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N112_RSMALL"] = str(int(rsmall))
    env["MYEVS_N112_THRESH"] = str(float(thresh))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"sweep failed for rsmall={rsmall}, thresh={thresh}: return code {proc.returncode}"
        )

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n112_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n112 RSMALL/THRESH via slim sweep and aggregate metrics.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n112_params")

    ap.add_argument("--rsmall-grid", default="1,2")
    ap.add_argument("--thresh-grid", default="0.6,0.7,0.8")
    ...
    rsmall_grid = parse_int_list(args.rsmall_grid)
    thresh_grid = parse_float_list(args.thresh_grid)

    results: list[dict] = []
    for rsmall in rsmall_grid:
        for thresh in thresh_grid:
            name = f"rs{int(rsmall)}_th{fmt_float(float(thresh))}"
    ...
    if args.full_validate:
        top_rsmall = int(best["rsmall"])
        top_thresh = float(best["thresh"])
        name = str(best["name"])
