...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N127_ALPHA1"] = str(float(alpha1))
    env["MYEVS_N127_TAU1_US"] = str(int(tau1_us))
    env["MYEVS_N127_ALPHA2"] = str(float(alpha2))
    env["MYEVS_N127_TAU2_US"] = str(int(tau2_us))
    env["MYEVS_N127_RAW_THR"] = str(float(raw_thr))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"sweep failed for a1={alpha1}, t1={tau1_us}, a2={alpha2}, t2={tau2_us}, raw_thr={raw_thr}: "
            f"return code {proc.returncode}"
        )

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n127_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n127 top1+top2 bonus params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n127_top2_bonus")

    ap.add_argument("--alpha1-grid", default="0.1,0.2,0.3")
    ap.add_argument("--tau1-us-grid", default="2000,5000,10000")
    ap.add_argument("--alpha2-grid", default="0.05,0.1,0.2")
    ap.add_argument("--tau2-us-grid", default="5000,10000,20000")
    ap.add_argument("--raw-thr-grid", default="2.0")
    ...
    alpha1_grid = parse_float_list(args.alpha1_grid)
    tau1_grid = parse_int_list(args.tau1_us_grid)
    alpha2_grid = parse_float_list(args.alpha2_grid)
    tau2_grid = parse_int_list(args.tau2_us_grid)
    raw_thr_grid = parse_float_list(args.raw_thr_grid)

    results: list[dict] = []
    for alpha1 in alpha1_grid:
        for tau1_us in tau1_grid:
            for alpha2 in alpha2_grid:
                for tau2_us in tau2_grid:
                    for raw_thr in raw_thr_grid:
                        name = (
                            f"a1{fmt_f(alpha1)}_t1{int(tau1_us)}_"
                            f"a2{fmt_f(alpha2)}_t2{int(tau2_us)}_r{fmt_f(raw_thr)}"
                        )
    ...
    if args.full_validate:
        ...
        full = run_one(
            ...
            tau1_us=int(best["tau1_us"]),
            alpha2=float(best["alpha2"]),
            tau2_us=int(best["tau2_us"]),
            raw_thr=float(best["raw_thr"]),
