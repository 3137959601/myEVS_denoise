"""Fix YNoise on ED24 Bicycle: r×tau sweep per level, 12-threaded."""
...
def run_one(lv, clean, noisy, r, tau):
    csv = f"{OUT}/_ynoise_cache/ynoise_{lv}_r{r}_tau{tau}.csv"
...
total = len(tasks)
print(f"YNoise Bike fix: {total} tasks ({len(LEVELS)}lv x {len(R_LIST)}r x {len(TAU_LIST)}tau)")
t0 = time.time()
...
df.to_csv(f"{OUT}/ynoise_bike_v2_all.csv", index=False)

print("\n=== YNOISE BIKE PER-LEVEL BEST ===")
...
# Consistency check at each (r,tau)
print("\n=== CONSISTENCY CHECK ===")
anomalies = 0
for r in R_LIST:
    for tau in TAU_LIST:
        ...
        if len(row_18) and len(row_33):
            ...
            if a33 > a18:
                anomalies += 1
                print(f"  ANOMALY r={r} tau={tau}: light={a18:.4f} heavy={a33:.4f}")
print(f"  {anomalies}/{len(R_LIST)*len(TAU_LIST)} anomalies total")
