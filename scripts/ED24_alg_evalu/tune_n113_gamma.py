...
def parse_float_list(s: str) -> list[float]:
    ...
    for part in str(s).split(","):
        ...
        out.append(float(p))
    if not out:
        raise ValueError("empty gamma list")
    return out


def fmt_gamma(v: float) -> str:
...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N113_GAMMA"] = str(float(gamma))
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"sweep failed for gamma={gamma}: return code {proc.returncode}")

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n113_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n113 GAMMA via slim sweep and aggregate metrics.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n113_gamma")

    ap.add_argument("--gamma-grid", default="1.0,1.5,2.0,2.5,3.0,4.0")
    ...
    for gamma in gamma_grid:
        ...
        rec["name"] = name
        results.append(rec)
        print(f"[OK] {name} mean_auc={rec['mean_auc']:.6f} mean_f1={rec['mean_f1']:.6f} elapsed={rec['elapsed_sec']:.1f}s")
