...
def run_one(
    *,
    ...
    env = os.environ.copy()
    env["MYEVS_N118_ECHO_MIN_US"] = str(int(echo_min_us))
    env["MYEVS_N118_ECHO_MAX_US"] = str(int(echo_max_us))
    env["MYEVS_N118_ALPHA"] = str(float(alpha))
    env["MYEVS_N118_DIST_TH"] = str(float(dist_th))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "sweep failed for "
            f"echo_min_us={echo_min_us}, echo_max_us={echo_max_us}, alpha={alpha}, dist_th={dist_th}: "
            f"return code {proc.returncode}"
        )

    per_env: dict[str, dict[str, float | str]] = {}
    for env_name in ENV_NAMES:
        matches = glob.glob(str(out_dir / f"roc_ebf_n118_{env_name}_labelscore_*.csv"))
        if len(matches) != 1:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Tune n118 dipole params via slim sweep.")
    ap.add_argument("--python-exe", default="")
    ap.add_argument("--project-root", default=".")
    ap.add_argument("--base-out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_tune_n118_dipole")

    ap.add_argument("--echo-min-us", type=int, default=1000)
    ap.add_argument("--echo-max-us", type=int, default=60000)
    ap.add_argument("--alpha-grid", default="0.5,1.0,2.0")
    ap.add_argument("--dist-th-grid", default="1.0,1.5,2.0")
    ...
    dist_th_grid = parse_float_list(args.dist_th_grid)

    results: list[dict] = []
    for alpha in alpha_grid:
        for dist_th in dist_th_grid:
            name = (
                f"emin{int(args.echo_min_us)}_emax{int(args.echo_max_us)}"
                f"_a{fmt_f(float(alpha))}_d{fmt_f(float(dist_th))}"
            )
            ...
            rec = run_one(
                ...
                out_dir=out_dir,
                echo_min_us=int(args.echo_min_us),
                echo_max_us=int(args.echo_max_us),
                alpha=float(alpha),
                dist_th=float(dist_th),
