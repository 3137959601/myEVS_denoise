...
def _require_numba() -> None:
    if numba is None:
        raise SystemExit("gt_feature_stats_783_v2 requires numba")
...
def _kernel_783_v2(
    ...
    for i in range(n):
        ...
        if s_base >= score_thr:
            ...
            # Revised formula from 7.83 update: normalize by full baseline support.
            a = emax / (s_base + 1e-3)
            if a < 0.0:
                a = 0.0
            if a > 1.0:
...
def run_783_v2(
    *,
    ...
    for cls_name, cval in (("noise", 0), ("signal", 1)):
        ...
        l0_0p2 = float(np.mean((v >= 0.0) & (v < 0.2))) if cnt > 0 else float("nan")
        r0p8_1 = float(np.mean((v >= 0.8) & (v <= 1.0))) if cnt > 0 else float("nan")
        ...
        rows_summary.append(
            {
                ...
                "a_true_p50": p50,
                "a_true_p90": p90,
                "a_true_0_0p2_ratio": l0_0p2,
                "a_true_0p8_1_ratio": r0p8_1,
            }
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="7.83 v2 stats: revised anisotropy A_true histogram")
    ap.add_argument(
        "--labeled-npy",
        default="D:/hjx_workspace/scientific_reserach/dataset/ED24/myPedestrain_06/Pedestrain_06_3.3.npy",
    )
    ap.add_argument("--out-dir", default="data/ED24/myPedestrain_06/EBF_Part2/_gt_feature_stats_783v2_heavy")
