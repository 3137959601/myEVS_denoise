...
def _algo_space(*, evflow_lite: bool) -> list[AlgoCfg]:
    return [
        AlgoCfg("baf", "baf", "cpp", [1], [2000, 4000, 8000, 16000], [1.0]),
        # Align STCF_orig tau grid with BAF grid for fair comparison.
        AlgoCfg("stcf_orig", "stcf_original", "cpp", [1], [2000, 4000, 8000, 16000], [1, 2, 3, 4, 5, 6]),
        ...
        AlgoCfg("pfd", "pfd", "cpp", [1], [8000, 16000, 32000], [1, 2, 3, 4, 5, 6, 8]),
    ]
...
def main() -> int:
    ...
    for ai, alg in enumerate(algs, start=1):
        ...
        total = len(alg.radii) * len(alg.taus_us) * len(pfd_m_list)
        done = 0
        for r in alg.radii:
            for tau in alg.taus_us:
                for m in pfd_m_list:
                    ...
                    if m is None:
                        ...
                        print(f"[DVSCLEAN-sweep][{alg.name}] {done}/{total} r={r} tau={tau} m={m}", flush=True)
                        csv_path = out_dir / alg.name / f"roc_{alg.name}_r{r}_tau{tau}_m{m}.csv"
                        tag = f"{alg.name}_r{r}_tau{tau}_m{m}_{scene}_{level}"
