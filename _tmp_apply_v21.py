with open('cpp/native_emlb/native_emlb.hpp', 'r', encoding='utf-8-sig') as f:
    content = f.read()

# Find N149Native class boundaries
start = content.find('class N149Native : public NativeBase {')
# Find the closing }; before namespace close
ns_close = content.rfind('\n} // namespace')
before_ns = content[:ns_close]
end = before_ns.rfind('\n};')

print(f'N149Native: pos {start} to {end} ({(content[:start].count(chr(10)))+1} to {(content[:end].count(chr(10)))+2})')

# Build v2.1 replacement
v21 = '''class N149Native : public NativeBase {
public:
    N149Native(int width, int height, uint64_t duration_ticks, int radius, double threshold,
               double sigma = 3.0, bool show_on = true, bool show_off = true)
        : NativeBase(width, height, show_on, show_off),
          tau_(std::max<uint64_t>(1, duration_ticks)),
          radius_(std::clamp(radius, 0, 8)),
          threshold_(threshold),
          sigma_(std::max(0.1, sigma)),
          ...
          alpha_instant_(env_flag("MYEVS_N149_ALPHA_INSTANT")),
...
private:
    ...
    bool hot_binary_, hot_lut_enabled_, wt_linear_, simple_a_, simple_b_, alpha_instant_;
    ...
    static double read_alpha_fixed() {
        const char *v = std::getenv("MYEVS_N149_ALPHA_FIXED"); if (!v) return -1.0;
        try { double d = std::stod(v); return std::isfinite(d) && d >= 0.0 && d <= 1.0 ? d : -1.0; } catch (...) { return -1.0; }
    }
...
# Separate acc_neighbor and score_one — they contain raw C++ that needs proper formatting
rest = '''
    ...
    double score_one(int32_t x, int32_t y, int8_t p, uint64_t t) {
        ...
        double m_eff = alpha_instant_ ? (raw_same+raw_opp > 0 ? raw_opp/(raw_same+raw_opp+1e-6) : 0.0) : mix_state_;
        double alpha_eff;
        if (alpha_fixed_ >= 0.0) alpha_eff = alpha_fixed_;
        else {
...
new_content = content[:start] + v21 + rest + content[end+3:]
with open('cpp/native_emlb/native_emlb.hpp', 'w', encoding='utf-8') as f:
    f.write(new_content)
